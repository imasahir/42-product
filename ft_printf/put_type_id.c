...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/08/31 21:04:18 by imasahir          #+#    #+#             */
/*   Updated: 2020/09/05 21:30:24 by imasahir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			put_digit_id(char *str, int s_len, int len, t_printf *p_info)
{
	if (p_info->minus == ON)
	{
		p_info->len += put_digit_id2(str, s_len, len, p_info);
		if (p_info->field > p_info->prec)
			put_space_id(str, s_len, p_info);
	}
	else
	{
		put_space_id(str, s_len, p_info);
...
int			put_type_id(long i, t_printf *p_info)
{
	...
	put_digit_id(str, s_len, len, p_info);
	safe_free(str);
	return (p_info->len);
}
