...
/*   Updated: 2020/09/05 21:31:00 by imasahir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				put_digit_x(char *str, int s_len, int len, t_printf *p_info)
{
	if (p_info->minus == ON)
	{
		p_info->len += put_digit_x2(str, s_len, len, p_info);
		if (p_info->field > p_info->prec)
			put_space_x(str, s_len, p_info);
	}
	else
	{
		put_space_x(str, s_len, p_info);
...
int				put_type_x(unsigned int id, t_printf *p_info)
{
	...
	if (!(str = ft_itoa_base(id, "0123456789ABCDEF")))
		return (ERROR);
	if (p_info->type == 'x')
		ft_strlower(str);
	len = ft_strlen(str);
	...
	put_digit_x(str, s_len, len, p_info);
	safe_free(str);
	return (p_info->len);
}
